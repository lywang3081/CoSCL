...
class Appr(object):
    ...
    def train(self, t, xtrain, ytrain, xvalid, yvalid, data, input_size, taskcla):
        ...
        if t>0:
            ...
            for name, param in self.model.named_parameters():
                if 'bias' in name or 'last' in name or 'eg' in name:
                    continue
                key = name.split('.')[0]
        ...
        temp=utils.gs_cal_mmoe(t,xtrain,ytrain,self.criterion, self.model)  #calculate omega
    ...
    def train_epoch(self,t,x,y,lr):
        ...
        for i in range(0,len(r),self.sbatch):
            ...
            if t>0:
                for name, param in self.model.named_parameters():
                    if 'bias' in name or 'last' in name or 'eg' in name or 'conv1' in name:
                        continue
                    key = name.split('.')[0]
    ...
    def proxy_grad_descent(self, t, lr):
        with torch.no_grad():
            ...
            #for name, module in self.model.named_children():
                ...
                bias = module.bias
                weight_old = module_old.weight #self.old_module[name].weight #
                bias_old = module_old.bias #self.old_module[name].bias #

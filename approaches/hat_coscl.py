...
class Appr(object):
    ...
    def __init__(self,model,nepochs=100,sbatch=256,lr=0.05,lr_min=1e-4,lr_factor=3,
                 lr_patience=5,clipgrad=10000,lamb=0.75,smax=400,args=None,log_name = None, use_TG=False):
        self.model=model
    ...
    def train_epoch(self,t,x,y,thres_cosh=50,thres_emb=6):
        ...
        for i in range(0,len(r),self.sbatch):
            ...
            # Forward
            output,masks, outputs_expert, logit_expert=self.model.forward(task,images,s=s, return_expert=True)
            output=output[t]
    ...
    def eval(self,t,x,y):
        ...
        for i in range(0,len(r),self.sbatch):
            ...
            # Forward
            output, masks, outputs_expert, logit_expert=self.model.forward(task,images,s=self.smax, return_expert=True)
            output=output[t]

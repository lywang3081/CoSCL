...
def gs_cal_mmoe(t, x, y, criterion, model, sbatch=20):
    # Init
    param_R = {}
    ...
    for i in range(0, x.size(0), sbatch):
        ...
        outputs = model.forward(images, task, avg_act=True)
        cnt = 0
        ...
        for name, param in model.named_parameters():
            if len(param.size()) <= 1 or 'last' in name or 'downsample' in name or 'eg' in name:
                continue
            name = name.split('.')[:-1]
            name = '.'.join(name)
            param_R[name] += model.act[cnt].abs().detach() * sbatch
...
def print_model_report(model):
    ...
    print()
    print('Num all parameters = %s'%(human_format(count_all)))
    print('-'*100)
    return count
...
def fisher_matrix_diag_coscl(t,x,y,model,criterion,sbatch=20):
    # Init
    fisher={}
    for n,p in model.named_parameters():
        fisher[n]=0*p.data
...
def fisher_matrix_diag_coscl_ce(t,x,y,model,criterion,sbatch=20):
    # Init
    fisher={}
    for n,p in model.named_parameters():
        fisher[n]=0*p.data
    ...
    for i in tqdm(range(0,x.size(0),sbatch),desc='Fisher diagonal',ncols=100,ascii=True):
        ...
        outputs_expert, outputs, logit_expert = model.forward(images, task, return_expert=True)

        loss = 0
        for exp in range(model.nExpert):
            loss += criterion(outputs_expert[exp], target) / model.nExpert
...
def gradient_direction_matrix(t,x,y,model,criterion,sbatch=20):
    # Init
    fisher={}
    for n,p in model.named_parameters():
        fisher[n]=0*p.data
    ...
    # Compute
    model.train()
    for i in tqdm(range(0,x.size(0),sbatch),desc='gradient_direction_matrix',ncols=100,ascii=True):
        b=torch.LongTensor(np.arange(i,np.min([i+sbatch,x.size(0)]))).cuda()
        images=x[b]
        targets=y[b]
        task = torch.autograd.Variable(torch.LongTensor([t]).cuda())
...
def fisher_matrix_diag_pg(t,x,y,model,criterion,sbatch=20):
    # Init
    fisher={}
    for n,p in model.named_parameters():
        fisher[n]=0*p.data
